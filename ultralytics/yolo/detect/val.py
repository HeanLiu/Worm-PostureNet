...
class DetectionValidator(BaseValidator):
    ...
    #         else:
    #             print(f"Warning: Key '{k}' has an empty list of tensors. Skipping.")
    #             stats[k] = np.array([])  # 将空列表替换为空 NumPy 数组
    #
    #     # 检查关键数据是否为空
    #     if "tp" not in stats or stats["tp"].size == 0:
    #         print("Warning: No true positives (tp) found. Metrics cannot be calculated.")
    #         return {}  # 返回空字典，跳过指标计算
    #
    #     if "conf" not in stats or stats["conf"].size == 0:
    #         print("Warning: No confidence scores (conf) found. Metrics cannot be calculated.")
    #         return {}  # 返回空字典，跳过指标计算
    #
    #     if "pred_cls" not in stats or stats["pred_cls"].size == 0:
    #         print("Warning: No predicted classes (pred_cls) found. Metrics cannot be calculated.")
    #         return {}  # 返回空字典，跳过指标计算
    #
    #     # 处理非空数据
    #     if "target_cls" in stats and len(stats["target_cls"]) > 0:
    #         self.nt_per_class = np.bincount(stats["target_cls"].astype(int), minlength=self.nc)
    #     else:
    #         self.nt_per_class = np.zeros(self.nc, dtype=int)  # 如果 target_cls 为空，初始化为全零数组
    #
    #     if "target_img" in stats and len(stats["target_img"]) > 0:
    #         self.nt_per_image = np.bincount(stats["target_img"].astype(int), minlength=self.nc)
    #     else:
    #         self.nt_per_image = np.zeros(self.nc, dtype=int)  # 如果 target_img 为空，初始化为全零数组
    #
    #     stats.pop("target_img", None)  # 移除 target_img 键
    #
    #     if len(stats):  # 如果 stats 非空
    #         try:
    #             self.metrics.process(**stats, on_plot=self.on_plot)
